...
export interface BillsPayServiceData {
  serviceID: string
  name: string
  category: string
  provider: string
...
export interface BillsPayVariation {
  variation_id: string
  service_name: string
  service_id: string
  data_plan?: string
...
export interface BillsPayTransaction {
  code: string
...
export class BillsPayService {
  private readonly logger = new Logger(BillsPayService.name)
  private readonly baseUrl = 'https://api.billspay.com/v1'
  ...
  constructor(
    ...
    this.apiKey = this.configService.get<string>('BILLSPAY_API_KEY') || ''
    this.secretKey = this.configService.get<string>('BILLSPAY_SECRET_KEY') || ''
  }

  /**
   * Get all available services from BillsPay
   */
  async getServices(): Promise<BillsPayServiceData[]> {
    try {
      if (!this.apiKey) {
        this.logger.warn('BILLSPAY_API_KEY not configured, using mock services')
      ...
      this.logger.error('Failed to fetch BillsPay services:', error)
   ...
   */
  async getVariations(serviceID: string): Promise<BillsPayVariation[]> {
    try {
      if (!this.apiKey) {
        this.logger.warn('BILLSPAY_API_KEY not configured, using mock variations')
  ...
  ): Promise<BillsPayTransaction> {
    try {
      if (!this.apiKey) {
        this.logger.warn('BILLSPAY_API_KEY not configured, using mock purchase response')
      ...
      this.logger.log(`Initiating BillsPay purchase: ${serviceID} for ${phone || 'N/A'}`, payload)
      ...
      this.logger.error('BillsPay purchase failed:', error)
   ...
   */
  async queryTransaction(requestId: string): Promise<BillsPayTransaction> {
    try {
      if (!this.apiKey) {
        this.logger.warn('BILLSPAY_API_KEY not configured, using mock transaction response')
  ...
  async getPricing(serviceType: string, network: string, amount?: number): Promise<any> {
    try {
      if (!this.apiKey) {
        this.logger.warn('BILLSPAY_API_KEY not configured, using mock pricing')
  ...
  async getAvailability(serviceType: string, network: string): Promise<boolean> {
    try {
      if (!this.apiKey) {
        this.logger.warn('BILLSPAY_API_KEY not configured, assuming available for development')
        return true
      }
      const response: AxiosResponse = await firstValueFrom(
  ...
  /**
   * Webhook handler for BillsPay transaction updates
   */
  async handleWebhook(payload: any): Promise<void> {
    this.logger.log('BillsPay webhook received:', payload)
  ...
  private generateRequestId(): string {
    ...
    return `dorce_billspay_${timestamp}_${random}`
  }
  ...
  /**
   * Verify webhook signature from BillsPay
   */
  private verifyWebhookSignature(payload: any): boolean {
    // Implementation will depend on BillsPay webhook signature method
   ...
   */
  private getMockServices(): BillsPayServiceData[] {
    return [
      {
        serviceID: 'MTN_AIRTIME',
        name: 'MTN Airtime',
        category: 'airtime',
        provider: 'MTN',
        commission: 0.035,
   ...
   */
  private getMockVariations(serviceID: string): BillsPayVariation[] {
    const variations = {
      'MTN_AIRTIME': [
        { variation_id: 'mtn_100', service_name: 'MTN ₦100', service_id: 'MTN_AIRTIME', airtime_amount: '100', price: '100', availability: 'Available' as const },
        { variation_id: 'mtn_500', service_name: 'MTN ₦500', service_id: 'MTN_AIRTIME', airtime_amount: '500', price: '500', availability: 'Available' as const },
        { variation_id: 'mtn_1000', service_name: 'MTN ₦1000', service_id: 'MTN_AIRTIME', airtime_amount: '1000', price: '1000', availability: 'Available' as const },
      ],
      'AIRTEL_AIRTIME': [
        { variation_id: 'airtel_100', service_name: 'Airtel ₦100', service_id: 'AIRTEL_AIRTIME', airtime_amount: '100', price: '100', availability: 'Available' as const },
        { variation_id: 'airtel_500', service_name: 'Airtel ₦500', service_id: 'AIRTEL_AIRTIME', airtime_amount: '500', price: '500', availability: 'Available' as const },
        { variation_id: 'airtel_1000', service_name: 'Airtel ₦1000', service_id: 'AIRTEL_AIRTIME', airtime_amount: '1000', price: '1000', availability: 'Available' as const },
      ],
    }
   ...
   */
  private getMockPurchaseResponse(serviceID: string, amount: number, phone?: string, variation_id?: string): BillsPayTransaction {
    return {
      code: '000',
   ...
   */
  private getMockTransactionResponse(requestId: string): BillsPayTransaction {
    return {
      code: '000',
      response_description: 'Transaction successful',
      requestId: requestId,
